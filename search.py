...
def abstractSearch(G:nx.DiGraph, start:str, method:Method) -> tuple[list[tuple[str,str]],list[str]]:
    """
    Abstract method for searching a graph using queue
    """
    L = list() #List of traversed vertices
    A = list() #List of traversed edges
    Q = [(None,start)] #Queue of edges for next search 
    # latex text for describing the searching process
    text = [f'0&&$\\emptyset$&$[{start}]$\\\\']
    count = 1
    p = 0
    ...
    while len(Q) > 0:
        ...
        if v not in L:# the destination is not traversed
            ...
            L.append(v)
        ss = f'{count}&${v}$&$[{List2Str(L)}]$&$[{List2Str(Q)}]$\\\\'
        text.append(ss.replace('v', 'v_'))
        count += 1
    return A, text

def BFS(G:nx.DiGraph, start:str) -> tuple[list[tuple[str,str]],list[str]]:
    """
...
def isConnected(G:nx.DiGraph, start:str, destination:str) -> tuple[bool,list[tuple[str,str]]]:
    """
...
def DFS(G:nx.DiGraph, start:str) -> list[tuple[str,str]]:
    """
    Depth First Search
...
def _DFS_search(G:nx.DiGraph, v:str, VisitedNode:list[str], UsedEdges:list[tuple[str,str]]):
    """
